...
- **IDE after MCP (same kernel)**: If MCP already started a kernel for a notebook and it is registered, selecting **JupyLink** in the IDE can **bridge** to that process instead of starting a second one. On startup, `python -m jupylink` checks reuse rules and, if matched, runs a ZMQ proxy that re-signs messages between the IDE’s connection file and the existing kernel. The bridge process **exits when the upstream kernel shuts down** (so **Restart kernel** / shutdown in Cursor complete normally). Set `JUPYLINK_IDE_REUSE=0` on the kernel process to force a normal local JupyLink kernel.
- **Run All / responsiveness**: The JupyLink kernel writes `_record.*` after each execute; that work runs in a **background thread** so the asyncio loop is not blocked—**Run All** and rapid executes stay responsive.
